...
Received: from bombadil.infradead.org (bombadil.infradead.org [IPv6:2607:7c80:54:e::133])
	by mail.lfdr.de (Postfix) with ESMTPS id A3704B804A
	for <lists+linux-arm-kernel@lfdr.de>; Thu, 19 Sep 2019 19:44:17 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed;
	...
	List-Owner; bh=XGIPlxx4MIXimzjb9PhNJ+Z/l7fhjlJLydezRXE8HVU=; b=qWt7gDAaAHGf2+
	;
Received: from localhost ([127.0.0.1] helo=bombadil.infradead.org)
	by bombadil.infradead.org with esmtp (Exim 4.92.2 #3 (Red Hat Linux))
	id 1iB0TF-0006II-PI; Thu, 19 Sep 2019 17:43:57 +0000
Received: from honk.sigxcpu.org ([24.134.29.49])
 by bombadil.infradead.org with esmtps (Exim 4.92.2 #3 (Red Hat Linux))
 id 1iB0Sm-0006HU-Jf
 for linux-arm-kernel@lists.infradead.org; Thu, 19 Sep 2019 17:43:33 +0000
...
X-CRM114-CacheID: sfid-20190919_104329_219575_4037E71F 
...
> On 14.09.2019 18:11, Guido G=FCnther wrote:
> > Hi Andrzej,
...
> >>>  obj-y +=3D synopsys/
> >>> diff --git a/drivers/gpu/drm/bridge/nwl-dsi/Kconfig b/drivers/gpu/drm=
/bridge/nwl-dsi/Kconfig
...
> >>> +
> >>> diff --git a/drivers/gpu/drm/bridge/nwl-dsi/Makefile b/drivers/gpu/dr=
m/bridge/nwl-dsi/Makefile
> >>> new file mode 100644
...
> >>> diff --git a/drivers/gpu/drm/bridge/nwl-dsi/nwl-drv.c b/drivers/gpu/d=
rm/bridge/nwl-dsi/nwl-drv.c
> >>> new file mode 100644
...
> >>> +	struct nwl_dsi_plat_clk_config clk_config[NWL_DSI_MAX_PLATFORM_CLOC=
KS];
> >>> +};
> >>
...
- This would only leave poweron/off which is different between imx8mq
  ...
  https://source.codeaurora.org/external/imx/linux-imx/tree/drivers/gpu/drm=
/imx/nwl_dsi-imx.c?h=3Dimx_4.14.78_1.0.0_ga#n419
...
> >>> +
> >>> +static inline struct nwl_dsi *bridge_to_dsi(struct drm_bridge *bridg=
e)
> >>> +{
> >>> +	return container_of(bridge, struct nwl_dsi, bridge);
> >>> +}
> >>> +
> >>> +static int nwl_dsi_set_platform_clocks(struct nwl_dsi *dsi, bool ena=
ble)
> >>> +{
...
> >>> +	adjusted_mode->flags |=3D (DRM_MODE_FLAG_PHSYNC | DRM_MODE_FLAG_PVS=
YNC);
> >>> +	adjusted_mode->flags &=3D ~(DRM_MODE_FLAG_NHSYNC | DRM_MODE_FLAG_NV=
SYNC);
> >>> +
> >>> +	return true;
> >>> +}
...
> >>> +	 * derived from it atm.
> >>> +	 */
> >>> +	if (new_cfg.mipi_dphy.hs_clk_rate =3D=3D dsi->phy_cfg.mipi_dphy.hs_=
clk_rate)
> >>> +		return;
> >>> +
...
> >>> +		use_dcss =3D 0;
> >>
> >> Relying on node name seems to me wrong. I am not sure if whole logic f=
or
...
https://source.puri.sm/guido.gunther/linux-imx8/blob/forward-upstream/next-=
20190823/mxsfb+nwl/v4/arch/arm64/boot/dts/freescale/imx8mq.dtsi#L477
...
> >>> diff --git a/drivers/gpu/drm/bridge/nwl-dsi/nwl-drv.h b/drivers/gpu/d=
rm/bridge/nwl-dsi/nwl-drv.h
> >>> new file mode 100644
...
> >>> +
> >>> +#endif /* __NWL_DRV_H__ */
> >>> diff --git a/drivers/gpu/drm/bridge/nwl-dsi/nwl-dsi.c b/drivers/gpu/d=
rm/bridge/nwl-dsi/nwl-dsi.c
> >>> new file mode 100644
...
> >>> +}
> >>> +
> >>> +static u32 nwl_dsi_get_dpi_pixel_format(enum mipi_dsi_pixel_format f=
ormat)
> >>> +{
> >>> +	switch (format) {
...
> >>> +{
> >>> +	u32 cycles;
> >>> +	struct phy_configure_opts_mipi_dphy *cfg =3D &dsi->phy_cfg.mipi_dph=
y;
> >>> +
> >>> +	if (dsi->lanes < 1 || dsi->lanes > 4)
> >>> +		return -EINVAL;
...
> >>> +
> >>> +	nwl_dsi_write(dsi, NWL_DSI_INTERFACE_COLOR_CODING, NWL_DSI_DPI_24_B=
IT);
...
> >>> +{
> >>> +	struct nwl_dsi *dsi =3D container_of(dsi_host, struct nwl_dsi, dsi_=
host);
> >>> +	struct device *dev =3D dsi->dev;
> >>> +	struct drm_bridge *bridge;
...
> >>> +}
> >>> +
> >>> +static void nwl_dsi_finish_transmission(struct nwl_dsi *dsi, u32 sta=
tus)
> >>> +{
> >>> +	struct nwl_dsi_transfer *xfer =3D dsi->xfer;
...
> >>> +	return 0;
> >>> +}
> >>> diff --git a/drivers/gpu/drm/bridge/nwl-dsi/nwl-dsi.h b/drivers/gpu/d=
rm/bridge/nwl-dsi/nwl-dsi.h
> >>> new file mode 100644
