...
Received: from bombadil.infradead.org (bombadil.infradead.org [IPv6:2607:7c80:54:e::133])
	by mail.lfdr.de (Postfix) with ESMTPS id F0FFC10D7D0
	for <lists+linux-arm-kernel@lfdr.de>; Fri, 29 Nov 2019 16:21:59 +0100 (CET)
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed;
	...
	List-Owner; bh=Z+HhiTE+XeP2Gq/mHgK1FWmwZfB4/PwiuAO3aMJkyfE=; b=ueroM3gsnvsG1C
	;
Received: from localhost ([127.0.0.1] helo=bombadil.infradead.org)
	by bombadil.infradead.org with esmtp (Exim 4.92.3 #3 (Red Hat Linux))
	id 1iai5k-0003mD-9y; Fri, 29 Nov 2019 15:21:56 +0000
Received: from mail.nic.cz ([2001:1488:800:400::400])
 by bombadil.infradead.org with esmtps (Exim 4.92.3 #3 (Red Hat Linux))
 id 1iai5Z-0003lV-Ko
 for linux-arm-kernel@lists.infradead.org; Fri, 29 Nov 2019 15:21:48 +0000
Received: from dellmb (unknown [IPv6:2001:1488:fffe:6:cac7:3539:7f1f:463])
 by mail.nic.cz (Postfix) with ESMTPSA id 768BB140FC1;
 Fri, 29 Nov 2019 16:21:38 +0100 (CET)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=nic.cz; s=default;
 t=1575040898; bh=Z7OOxRCI8DLCgr7d+40cmmgvgUKCo8MD8oU7vkiWuQ0=;
 h=Date:From:To;
 b=e0de+Yo9Ya+4ENGuDNePOCdlFKGxlpLLXP/a/JIydantkR+MMA8rAtzZqrywPhPm/
 jpAqnjIuxyehigmGB/ECb6LQjRFuj+xx/Fl/4hJwcmyPuoQSlLrNMafhY20I/QDkoC
 OMKhS3PV6VMkVP3zCs6cSzZIhXVU4ZYMn2HQ8uIA=
Date: Fri, 29 Nov 2019 16:21:38 +0100
From: Marek =?ISO-8859-1?Q?Beh=FAn?= <marek.behun@nic.cz>
To: Leigh Brown <leigh@solinno.co.uk>
Subject: Re: Turris MOX Mainline Kernel
Message-ID: <20191129162138.392be91b@dellmb>
In-Reply-To: <8165fdeb5583069d893436e95cac5436@solinno.co.uk>
References: <8165fdeb5583069d893436e95cac5436@solinno.co.uk>
...
X-CRM114-CacheID: sfid-20191129_072146_136622_17D6CCEE 
...
I have compiled 5.4.1 with your configuration and it works. The only
differences are that I added BTRFS driver (since my root is on btrfs),
different boot command line and that it is 5.4.1 instead of 5.4.0.

My command line is
  earlycon=ar3700_uart,0xd0012000 console=ttyMV0,115200 rootwait
  rootfstype=btrfs root=PARTUUID=00000000-01
  rootflags=commit=5,subvol=/root

Looking at your kernel output it seems to me that kernel won't find
your SD card and because of the rootwait flag waits for the root
filesystem indefinitely.

Your output contains
  xenon-sdhci d00d0000.sdhci: allocated mmc-pwrseq
  xenon-sdhci d00d8000.sdhci: Got CD GPIO
but it should also contain
  mmc0: SDHCI controller on d00d0000.sdhci [d00d0000.sdhci] using ADMA
  mmc1: SDHCI controller on d00d8000.sdhci [d00d8000.sdhci] using ADMA
which it for some reason doesn't.

Marek

On Thu, 28 Nov 2019 16:28:40 +0000
Leigh Brown <leigh@solinno.co.uk> wrote:

> Hello,
> 
> I have a Turris MOX Classic and would like to run Debian Linux on it
> (it will replace my aging Mirabox).  I have installed Debian using 
...
> build my own kernel. I trimmed a lot of things out of the
> configuration in order to test it, with the aim of adding things back
> in once it was working.
> 
> The configuration is at 
> https://www.solinno.co.uk/public/moxboot/config-5.4.0
> 
> However, it is hanging during boot, a log of which I've pasted
> below. Is there a known-working config or a set of patches that I can
> use to obtain a working kernel (as new as possible, preferably)?
> 
...
>     Loading Device Tree to 000000001baba000, end 000000001bac1def ...
> OK
> 
> Starting kernel ...
> 
...
> [    0.000000] percpu: Embedded 16 pages/cpu s26648 r8192 d30696
> u65536 [    0.000000] Detected VIPT I-cache on CPU0
> [    0.000000] CPU features: detected: GIC system register CPU
> interface [    0.000000] Speculative Store Bypass Disable mitigation
> not required [    0.000000] Built 1 zonelists, mobility grouping on.
> Total pages: 128512
...
> [    0.012911] Calibrating delay loop (skipped), value calculated
> using timer frequency.. 25.00 BogoMIPS (lpj=125000)
> [    0.023557] pid_max: default: 32768 minimum: 301
> [    0.028451] Mount-cache hash table entries: 1024 (order: 1, 8192 
> bytes, linear)
> [    0.035907] Mountpoint-cache hash table entries: 1024 (order: 1,
> 8192 bytes, linear)
...
> [    0.226030] tcp_listen_portaddr_hash hash table entries: 256
> (order: 0, 4096 bytes, linear)
> [    0.234583] TCP established hash table entries: 4096 (order: 3,
> 32768 bytes, linear)
> [    0.242568] TCP bind hash table entries: 4096 (order: 4, 65536
> bytes, linear)
> [    0.249948] TCP: Hash tables configured (established 4096 bind
> 4096) [    0.256522] UDP hash table entries: 256 (order: 1, 8192
> bytes, linear)
> [    0.263181] UDP-Lite hash table entries: 256 (order: 1, 8192
> bytes, linear)
...
> [    0.502312] workingset: timestamp_bits=46 max_order=17
> bucket_order=0 [    0.513721] squashfs: version 4.0 (2009/01/31)
> Phillip Lougher [    0.524495] advk-pcie d0070000.pcie: host
> bridge /soc/pcie@d0070000 ranges:
...
> [    1.605462] pci_bus 0000:01: busn_res: [bus 01-ff] end is updated
> to 01
...
> [    1.709390] mv_xor d0060900.xor: Marvell XOR (Descriptor Mode):
> ( xor cpy intr )
> [    1.779372] mv_xor d0060900.xor: Marvell XOR (Descriptor Mode):
> ( xor cpy intr )
> [    1.787395] Serial: 8250/16550 driver, 16 ports, IRQ sharing
> enabled [    1.795922] cacheinfo: Unable to detect cache hierarchy
> for CPU 0 [    1.803309] moxtet spi0.1: Found MOX A (CPU) module
...
> [    1.845871] mvneta d0030000.ethernet eth0: Using hardware mac
> address d8:58:d7:00:c0:2a
...
> [    1.881134] orion-ehci d005e000.usb: new USB bus registered,
...
> [    1.933409] uhci_hcd: USB Universal Host Controller Interface
> driver [    1.940827] i2c /dev entries driver
