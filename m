Return-Path: <linux-arm-kernel-bounces+lists+linux-arm-kernel=lfdr.de@lists.infradead.org>
X-Original-To: lists+linux-arm-kernel@lfdr.de
Delivered-To: lists+linux-arm-kernel@lfdr.de
Received: from bombadil.infradead.org (bombadil.infradead.org [IPv6:2607:7c80:54:e::133])
	by mail.lfdr.de (Postfix) with ESMTPS id 4B9EC5FEFD
	for <lists+linux-arm-kernel@lfdr.de>; Fri,  5 Jul 2019 02:16:26 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed;
	d=lists.infradead.org; s=bombadil.20170209; h=Sender:Cc:List-Subscribe:
	List-Help:List-Post:List-Archive:List-Unsubscribe:List-Id:Content-Type:
	MIME-Version:Message-ID:Subject:To:From:Date:Reply-To:
	Content-Transfer-Encoding:Content-ID:Content-Description:Resent-Date:
	Resent-From:Resent-Sender:Resent-To:Resent-Cc:Resent-Message-ID:In-Reply-To:
	References:List-Owner; bh=fI1f9KXUyRVH3eR8cwTXgN0zbvOGin8/T6LHv4LyMQs=; b=GzX
	;
Received: from localhost ([127.0.0.1] helo=bombadil.infradead.org)
	by bombadil.infradead.org with esmtp (Exim 4.92 #3 (Red Hat Linux))
	id 1hjBtm-0001hm-Ex; Fri, 05 Jul 2019 00:16:22 +0000
Received: from mga04.intel.com ([192.55.52.120])
 by bombadil.infradead.org with esmtps (Exim 4.92 #3 (Red Hat Linux))
 id 1hjBtY-0001h1-Rl
 for linux-arm-kernel@lists.infradead.org; Fri, 05 Jul 2019 00:16:10 +0000
X-Amp-Result: UNKNOWN
X-Amp-Original-Verdict: FILE UNKNOWN
X-Amp-File-Uploaded: False
Received: from fmsmga002.fm.intel.com ([10.253.24.26])
 by fmsmga104.fm.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384;
 04 Jul 2019 17:16:07 -0700
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.63,452,1557212400"; 
 d="gz'50?scan'50,208,50";a="191481175"
Received: from lkp-server01.sh.intel.com (HELO lkp-server01) ([10.239.97.150])
 by fmsmga002.fm.intel.com with ESMTP; 04 Jul 2019 17:16:06 -0700
Received: from kbuild by lkp-server01 with local (Exim 4.89)
 (envelope-from <lkp@intel.com>)
 id 1hjBtV-000G2f-R0; Fri, 05 Jul 2019 08:16:05 +0800
Date: Fri, 5 Jul 2019 08:15:19 +0800
From: kbuild test robot <lkp@intel.com>
To: Christoffer Dall <christoffer.dall@arm.com>
Subject: [arm-platforms:kvm-arm64/nv-wip-5.2-rc6 98/102]
 arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:721:7: error: implicit
 declaration of function 'vcpu_el2_e2h_is_set'
Message-ID: <201907050816.3IlIapBC%lkp@intel.com>
MIME-Version: 1.0
Content-Type: multipart/mixed; boundary="T4sUOijqQbZv57TR"
Content-Disposition: inline
X-Patchwork-Hint: ignore
User-Agent: Mutt/1.5.23 (2014-03-12)
X-CRM114-Version: 20100106-BlameMichelson ( TRE 0.8.0 (BSD) ) MR-646709E3 
X-CRM114-CacheID: sfid-20190704_171608_928539_8A2AC41C 
X-CRM114-Status: GOOD (  11.19  )
X-Spam-Score: -2.3 (--)
X-Spam-Report: SpamAssassin version 3.4.2 on bombadil.infradead.org summary:
 Content analysis details:   (-2.3 points)
 pts rule name              description
 ---- ---------------------- --------------------------------------------------
 -2.3 RCVD_IN_DNSWL_MED      RBL: Sender listed at https://www.dnswl.org/,
 medium trust [192.55.52.120 listed in list.dnswl.org]
 0.0 SPF_HELO_NONE          SPF: HELO does not publish an SPF Record
 -0.0 SPF_PASS               SPF: sender matches SPF record
X-BeenThere: linux-arm-kernel@lists.infradead.org
X-Mailman-Version: 2.1.29
Precedence: list
List-Id: <linux-arm-kernel.lists.infradead.org>
List-Unsubscribe: <http://lists.infradead.org/mailman/options/linux-arm-kernel>, 
 <mailto:linux-arm-kernel-request@lists.infradead.org?subject=unsubscribe>
List-Archive: <http://lists.infradead.org/pipermail/linux-arm-kernel/>
List-Post: <mailto:linux-arm-kernel@lists.infradead.org>
List-Help: <mailto:linux-arm-kernel-request@lists.infradead.org?subject=help>
List-Subscribe: <http://lists.infradead.org/mailman/listinfo/linux-arm-kernel>, 
 <mailto:linux-arm-kernel-request@lists.infradead.org?subject=subscribe>
Cc: Marc Zyngier <marc.zyngier@arm.com>, kbuild-all@01.org,
 linux-arm-kernel@lists.infradead.org
Sender: "linux-arm-kernel" <linux-arm-kernel-bounces@lists.infradead.org>
Errors-To: linux-arm-kernel-bounces+lists+linux-arm-kernel=lfdr.de@lists.infradead.org


--T4sUOijqQbZv57TR
Content-Type: text/plain; charset=us-ascii
Content-Disposition: inline

tree:   https://kernel.googlesource.com/pub/scm/linux/kernel/git/maz/arm-platforms.git kvm-arm64/nv-wip-5.2-rc6
head:   a82a8fe3d83393e07c8452d27564dd34e0a20262
commit: 5f5b858bfc55e7cd80b3e122b719893baa8f9f2a [98/102] KVM: arm64: nv: Sync nested timer state with ARMv8.4
config: arm-axm55xx_defconfig (attached as .config)
compiler: arm-linux-gnueabi-gcc (GCC) 7.4.0
reproduce:
        wget https://raw.githubusercontent.com/intel/lkp-tests/master/sbin/make.cross -O ~/bin/make.cross
        chmod +x ~/bin/make.cross
        git checkout 5f5b858bfc55e7cd80b3e122b719893baa8f9f2a
        # save the attached .config to linux build tree
        GCC_VERSION=7.4.0 make.cross ARCH=arm 

If you fix the issue, kindly add following tag
Reported-by: kbuild test robot <lkp@intel.com>

All errors (new ones prefixed by >>):

   arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c: In function 'kvm_timer_sync_nested':
>> arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:721:7: error: implicit declaration of function 'vcpu_el2_e2h_is_set' [-Werror=implicit-function-declaration]
     if (!vcpu_el2_e2h_is_set(vcpu)) {
          ^~~~~~~~~~~~~~~~~~~
   In file included from arch/arm/include/asm/kvm_hyp.h:12:0,
                    from arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:17:
>> arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:728:20: error: implicit declaration of function '__vcpu_sys_reg'; did you mean 'vcpu_set_reg'? [-Werror=implicit-function-declaration]
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CTL_EL0),  SYS_CNTV_CTL);
                       ^
   arch/arm/include/asm/cp15.h:65:73: note: in definition of macro '__write_sysreg'
    #define __write_sysreg(v, r, w, c, t) asm volatile(w " " c : : "r" ((t)(v)))
                                                                            ^
   arch/arm/include/asm/kvm_hyp.h:86:33: note: in expansion of macro 'write_sysreg'
    #define write_sysreg_el0(v, r)  write_sysreg(v, r##_EL0)
                                    ^~~~~~~~~~~~
   arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:728:3: note: in expansion of macro 'write_sysreg_el0'
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CTL_EL0),  SYS_CNTV_CTL);
      ^~~~~~~~~~~~~~~~
>> arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:728:41: error: 'CNTV_CTL_EL0' undeclared (first use in this function); did you mean 'SYS_CNTV_CTL_EL0'?
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CTL_EL0),  SYS_CNTV_CTL);
                                            ^
   arch/arm/include/asm/cp15.h:65:73: note: in definition of macro '__write_sysreg'
    #define __write_sysreg(v, r, w, c, t) asm volatile(w " " c : : "r" ((t)(v)))
                                                                            ^
   arch/arm/include/asm/kvm_hyp.h:86:33: note: in expansion of macro 'write_sysreg'
    #define write_sysreg_el0(v, r)  write_sysreg(v, r##_EL0)
                                    ^~~~~~~~~~~~
   arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:728:3: note: in expansion of macro 'write_sysreg_el0'
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CTL_EL0),  SYS_CNTV_CTL);
      ^~~~~~~~~~~~~~~~
   arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:728:41: note: each undeclared identifier is reported only once for each function it appears in
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CTL_EL0),  SYS_CNTV_CTL);
                                            ^
   arch/arm/include/asm/cp15.h:65:73: note: in definition of macro '__write_sysreg'
    #define __write_sysreg(v, r, w, c, t) asm volatile(w " " c : : "r" ((t)(v)))
                                                                            ^
   arch/arm/include/asm/kvm_hyp.h:86:33: note: in expansion of macro 'write_sysreg'
    #define write_sysreg_el0(v, r)  write_sysreg(v, r##_EL0)
                                    ^~~~~~~~~~~~
   arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:728:3: note: in expansion of macro 'write_sysreg_el0'
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CTL_EL0),  SYS_CNTV_CTL);
      ^~~~~~~~~~~~~~~~
>> arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:729:41: error: 'CNTV_CVAL_EL0' undeclared (first use in this function); did you mean 'CNTV_CTL_EL0'?
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CVAL_EL0), SYS_CNTV_CVAL);
                                            ^
   arch/arm/include/asm/cp15.h:65:73: note: in definition of macro '__write_sysreg'
    #define __write_sysreg(v, r, w, c, t) asm volatile(w " " c : : "r" ((t)(v)))
                                                                            ^
   arch/arm/include/asm/kvm_hyp.h:86:33: note: in expansion of macro 'write_sysreg'
    #define write_sysreg_el0(v, r)  write_sysreg(v, r##_EL0)
                                    ^~~~~~~~~~~~
   arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:729:3: note: in expansion of macro 'write_sysreg_el0'
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CVAL_EL0), SYS_CNTV_CVAL);
      ^~~~~~~~~~~~~~~~
>> arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:730:41: error: 'CNTP_CTL_EL0' undeclared (first use in this function); did you mean 'CNTV_CTL_EL0'?
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTP_CTL_EL0),  SYS_CNTP_CTL);
                                            ^
   arch/arm/include/asm/cp15.h:65:73: note: in definition of macro '__write_sysreg'
    #define __write_sysreg(v, r, w, c, t) asm volatile(w " " c : : "r" ((t)(v)))
                                                                            ^
   arch/arm/include/asm/kvm_hyp.h:86:33: note: in expansion of macro 'write_sysreg'
    #define write_sysreg_el0(v, r)  write_sysreg(v, r##_EL0)
                                    ^~~~~~~~~~~~
   arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:730:3: note: in expansion of macro 'write_sysreg_el0'
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTP_CTL_EL0),  SYS_CNTP_CTL);
      ^~~~~~~~~~~~~~~~
>> arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:731:41: error: 'CNTP_CVAL_EL0' undeclared (first use in this function); did you mean 'CNTV_CVAL_EL0'?
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTP_CVAL_EL0), SYS_CNTP_CVAL);
                                            ^
   arch/arm/include/asm/cp15.h:65:73: note: in definition of macro '__write_sysreg'
    #define __write_sysreg(v, r, w, c, t) asm volatile(w " " c : : "r" ((t)(v)))
                                                                            ^
   arch/arm/include/asm/kvm_hyp.h:86:33: note: in expansion of macro 'write_sysreg'
    #define write_sysreg_el0(v, r)  write_sysreg(v, r##_EL0)
                                    ^~~~~~~~~~~~
   arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c:731:3: note: in expansion of macro 'write_sysreg_el0'
      write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTP_CVAL_EL0), SYS_CNTP_CVAL);
      ^~~~~~~~~~~~~~~~
   cc1: some warnings being treated as errors

vim +/vcpu_el2_e2h_is_set +721 arch/arm/kvm/../../../virt/kvm/arm/arch_timer.c

   711	
   712	void kvm_timer_sync_nested(struct kvm_vcpu *vcpu)
   713	{
   714		if (!is_hyp_ctxt(vcpu))
   715			return;
   716	
   717		/*
   718		 * Guest hypervisors using ARMv8.4 enhanced nested virt support have
   719		 * their EL1 timer register accesses redirected to the VNCR page.
   720		 */
 > 721		if (!vcpu_el2_e2h_is_set(vcpu)) {
   722			/*
   723			 * For a non-VHE guest hypervisor, we update the hardware
   724			 * timer registers with the latest value written by the guest
   725			 * to the VNCR page and let the hardware take care of the
   726			 * rest.
   727			 */
 > 728			write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CTL_EL0),  SYS_CNTV_CTL);
 > 729			write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTV_CVAL_EL0), SYS_CNTV_CVAL);
 > 730			write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTP_CTL_EL0),  SYS_CNTP_CTL);
 > 731			write_sysreg_el0(__vcpu_sys_reg(vcpu, CNTP_CVAL_EL0), SYS_CNTP_CVAL);
   732		} else {
   733			/*
   734			 * For a VHE guest hypervisor, the emulated state (which
   735			 * is stored in the VNCR page) could have been updated behind
   736			 * our back, and we must reset the emulation of the timers.
   737			 */
   738	
   739			struct timer_map map;
   740			get_timer_map(vcpu, &map);
   741	
   742			soft_timer_cancel(&map.emul_vtimer->hrtimer);
   743			soft_timer_cancel(&map.emul_ptimer->hrtimer);
   744			timer_emulate(map.emul_vtimer);
   745			timer_emulate(map.emul_ptimer);
   746		}
   747	}
   748	

---
0-DAY kernel test infrastructure                Open Source Technology Center
https://lists.01.org/pipermail/kbuild-all                   Intel Corporation

--T4sUOijqQbZv57TR
Content-Type: application/gzip
Content-Disposition: attachment; filename=".config.gz"
Content-Transfer-Encoding: base64



--T4sUOijqQbZv57TR
Content-Type: text/plain; charset="us-ascii"
MIME-Version: 1.0
Content-Transfer-Encoding: 7bit
Content-Disposition: inline

_______________________________________________
linux-arm-kernel mailing list
linux-arm-kernel@lists.infradead.org
http://lists.infradead.org/mailman/listinfo/linux-arm-kernel

--T4sUOijqQbZv57TR--

